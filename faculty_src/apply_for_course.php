<?php
    ...
    echo "<script> var no_course = 0;var apply_success = -1; </script>";

    if(isset($_SESSION['apply_success']))
    {
        if($_SESSION['apply_success'] == 1)
            echo "<script> apply_success = 1; </script>";
        else if($_SESSION['apply_success'] == 0)
            echo "<script> apply_success = 0; </script>";
...
<body style="background-color: rgb(255, 255, 128);">
    <div class="container-fluid pt-1">
        <div class="card">
            <div class="card-header p-3" style="text-align:center;display:inline;">
                <h1 style="font-family: Amita;"><b><i>Apply for Course</i></b></h1>
            ...
            <div class="card-body row">
                <div class="col-sm-3 list-group">
                    ...
                    <a href="apply_for_course.php" class="list-group-item list-group-item-action active">Apply for Courses</a>
                ...
                <div class="col-sm-9">
                    ...
                    <div class="alert alert-success" id="apply_success" style="display: none;">
                        Successfully Applied for Course
                    </div>
                    <div class="container">
                        <h2>Courses</h2>
    ...
    <?php
        ...
        $query1 = $conn->prepare("SELECT `course`.`course_id`,`course`.`admin_id`,`course`.`course_name`,`course`.`course_description`,`administrator`.`first_name`,`administrator`.`last_name`,`administrator`.`email`,`administrator`.`phone_no` FROM `course` INNER JOIN `administrator` ON `course`.`admin_id` = `administrator`.`admin_id` WHERE `course`.`faculty_id` IS NULL AND `course`.`course_id` NOT IN (SELECT `faculty_applied_courses`.`course_id` FROM `faculty_applied_courses` WHERE `faculty_applied_courses`.`faculty_id` = ?)");
...
<script>
    ...
    {
        var ip = '<?php echo $server_ip; ?>';
        window.location.href = 'http://'+ip+'/faculty_src/apply_for_course_2.php?id='+id;
    }
