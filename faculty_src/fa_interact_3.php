<?php
    ...
    if(isset($_POST['filetype']) && (isset($_POST['message']) || isset($_POST['file'])))
    {
        if($_POST['filetype'] == 'message')
        {
            ...
            $query0->bind_param("sssssi",$_SESSION['f_interact_course_id'],$_SESSION['f_interact_admin_id'],$_SESSION['id'],$_POST['message'],$interact_type,$interact_flag);
            if(!$query0->execute())
            {
        ...
        else
        {
            ...
            if($size < 40000000)
            {
                if(preg_match("/image/", $type) || preg_match("/pdf/",$type) || preg_match("/msword/",$type) || preg_match("/audio/",$type) || preg_match("/video/",$type))
                {
                    $x = rand(11111,99999);
                    $name = $x.$name;
                    $dir = "../courses/".$_SESSION['f_interact_course_id']."/fa_interact";
                    if ( !file_exists( $dir ) && !is_dir( $dir ) ) 
                    {
                        mkdir( $dir,0777,true);       
                    } 
                    $target_file = "../courses/".$_SESSION['f_interact_course_id']."/fa_interact/".$name;
                    if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file)) 
                    {
                        $interact_flag = 1;
                        $query1 = $conn->prepare("INSERT INTO `fa_interact` (`course_id`,`admin_id`,`faculty_id`,`message`,`msg_type`,`flag`) VALUES (?,?,?,?,?,?)");
                        $query1->bind_param("sssssi",$_SESSION['f_interact_course_id'],$_SESSION['f_interact_admin_id'],$_SESSION['id'],$name,$type,$interact_flag);
                        if(!$query1->execute())
                        {
                            $_SESSION['invalid_file'] = 3;
                        }
        ...
        header("Location:fa_interact_2.php");
    }
