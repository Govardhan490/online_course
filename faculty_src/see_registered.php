    ...
    <?php
        ...
        if($query1->execute())
        {
            ...
            if($query1->num_rows>0)
            {
                ...
                echo "\n});</script>";
                $query1->close();
                $query2 = $conn->prepare("SELECT `student`.`usn`,`course`.`course_id`,`student`.`first_name`,`student`.`last_name`,`student`.`email`,`student`.`phone_no` FROM ((`course` INNER JOIN `registered` ON `registered`.`course_id`=`course`.`course_id` AND `course`.`faculty_id` = ?)INNER JOIN `student` ON `registered`.`usn`=`student`.`usn`) ORDER BY `course`.`course_id`");
