<?php
    ...
    if(isset($_POST['interact']))
    {
        ...
        $_SESSION['s_interact_course_id'] = $value[0];
        $_SESSION['s_interact_admin_id'] = $value[1];
        $_SESSION['s_interact_course_name'] = $value[2];
        $_SESSION['s_interact_admin_name'] = $value[3];
    }
...
<head>
    ...
    <title>Student Admin Interaction</title>
...
<body style="background-color: rgb(255, 255, 128);">
    <div class="container-fluid pt-1">
        <div class="card">
            <div class="card-header p-3" style="text-align:center;display:inline;">
                <h1 style="font-family: Amita;"><b><i>Student Admin Interaction</i></b></h1>
            ...
            <div class="card-body row">
                ...
                <a href="view_registered_course.php" class="list-group-item list-group-item-action" style="color: black;">View Registered Courses</a>
                    ...
                    <a href="sa_interact.php" class="list-group-item list-group-item-action active">Interact with Admin</a>
                ...
                <div class="col-sm-9">
                    ...
                    <ul class="d-flex justify-content-center row list-group list-group-horizontal" style="text-align: center;margin:auto;">
                        <li class="col-sm-4 list-group-item"><b>Admin Name : </b><?php if(isset($_SESSION['s_interact_admin_name'])){echo $_SESSION['s_interact_admin_name'];} ?></li>
                        <li class="col-sm-5 list-group-item"><b>Course Name : </b><?php if(isset($_SESSION['s_interact_course_name'])){echo $_SESSION['s_interact_course_name'];} ?></li>
                    </ul>
                    <div class="card" style="overflow:scroll;height:400px;background-color:rgb(255, 255, 210);" id="chats">
                    </div>
                    <br>
                    <form action="sa_interact_3.php" method="post" onsubmit="return filesize_validate()" enctype="multipart/form-data">
    ...
    <?php
        ...
        $query3 = $conn->prepare("SELECT * FROM `sa_interact` WHERE `course_id` = ? ORDER BY `time_stamp`");
        $query3->bind_param("s",$_SESSION['s_interact_course_id']);
        if($query3->execute())
        {
            ...
            if($result->num_rows > 0)
            {
                ...
                while($data = $result->fetch_assoc())
                {
                    ...
                    if($data['flag'] == 0)
                    {
                        ...
                        {
                            echo "$('#chats').append(\"<div style='margin:10px;'><span style='padding:10px;background-color:white;margin:2px;font-size:20px;'><a href='../courses/$data[course_id]/sa_interact/$data[message]' download>$msg</a><span style='color:red;font-size:13px;'>($time)</span></span></div>\");\n";
                        }
                    }
                    else if($data['flag'] == 1)
                    {
                        ...
                        {
                            echo "$('#chats').append(\"<div align='right' style='margin:10px;'><span style='padding:10px;background-color:rgb(252, 206, 255);margin:2px;font-size:20px;'><a href='../courses/$data[course_id]/sa_interact/$data[message]' download>$msg</a><span style='color:red;font-size:13px;'>($time)</span></span></div>\");\n";
                        }
                    }
                }
                echo "\n});</script>";
