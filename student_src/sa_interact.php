...
<body style="background-color: rgb(255, 255, 128);">
            ...
            <div class="card-body row">
                ...
                <div class="col-sm-9">
                    ...
                    <div class="alert alert-info" id="no_course" style="display: none;">
                        You are not registered to  any courses yet
                    </div>
                    <div class="container">
                        <h2>Courses</h2>
    ...
    <?php
        ...
        $query2 = $conn->prepare("SELECT `course`.`course_id`,`course`.`admin_id`,`course`.`course_name`,`administrator`.`first_name`,`administrator`.`last_name`,`administrator`.`email`,`administrator`.`phone_no` FROM `course` INNER JOIN `administrator` ON `course`.`admin_id` = `administrator`.`admin_id` AND `course`.`course_id` IN (SELECT `course_id` FROM `registered` WHERE `registered`.`usn` = ?)");
