...
<body style="background-color: rgb(255, 255, 128);">
    ...
    <?php
        ...
        $query2 = $conn->prepare("SELECT `course`.`course_id`,`course`.`faculty_id`,`course`.`course_name`,`faculty`.`first_name`,`faculty`.`last_name`,`faculty`.`email`,`faculty`.`phone_no` FROM `course` INNER JOIN `faculty` ON `course`.`faculty_id` = `faculty`.`faculty_id` AND `course`.`course_id` IN (SELECT `course_id` FROM `registered` WHERE `registered`.`usn` = ?)");
