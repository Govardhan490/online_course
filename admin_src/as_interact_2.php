<?php
    ...
    if(isset($_POST['interact']))
    {
        ...
        $_SESSION['interact_student_name'] = $value[3];
    }
    ...
    if(isset($_SESSION['invalid_file']))
    {
        ...
        else if($_SESSION['invalid_file'] == 4)
        {
            echo "<script> var invalid_file = 3; </script>";
            echo "afojksdnfjksdnjkfnsjkdbnf";
        }
...
<?php
    ...
    $query3 = $conn->prepare("SELECT * FROM `sa_interact` WHERE `course_id` = ? AND `usn`=? ORDER BY `time_stamp`");
    $query3->bind_param("ss",$_SESSION['interact_course_id'],$_SESSION['interact_usn']);
    if($query3->execute())
    {
